...
func _ready()->void:
	for c in get_children():
		if not c is PlanetLayer: continue
		var l := c as PlanetLayer
		layers.push_back(l)
	update()

func _process(delta):
	if is_spinning: texture_offset += spin * delta
...
func set_parameter(name:String, value:Variant)->void:
	for l in layers: l.set(name, value)

func set_shader_parameter(name:String, value:Variant)->void:
	for l in layers: l.material.set_shader_parameter(name, value)

func add_layer(layer:PlanetLayer)->void:
	if layers.has(layer) || layer.get_parent(): return
